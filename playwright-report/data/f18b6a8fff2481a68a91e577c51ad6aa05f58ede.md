...
- Name: judge-flow.spec.ts >> VAPOR Real-Time Financial Circuit Breaker & Partner Flow E2E >> Prava Partner Journey: Linq iMessage approval for high-value scenario
- Location: tests\e2e\judge-flow.spec.ts:87:7
...
Locator: getByText('DECISION: REQUIRES_LINQ_APPROVAL', { exact: true })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('DECISION: REQUIRES_LINQ_APPROVAL', { exact: true })
...
- main:
  ...
  - text: 1 EVENTS 8:36:15 AM PURCHASE_REQUESTED Employee initiated purchase for Datadog Enterprise Monitoring Tier ($4999.00 USD)
- alert
```
